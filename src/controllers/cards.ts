...
export const getCards = (req: Request, res: Response) => cardSchema.find({})
  ...
  .catch((err) => {
    ...
    return res.status(SERVER_ERROR).send({ message: 'Произошла ошибка при загрузке карточек' });
  });
...
export const toggleLike = (req: RequestWithUserRole, res: Response) => cardSchema.findByIdAndUpdate(
  req.params.cardId,
  req.method === 'PUT'
